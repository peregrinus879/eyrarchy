...
2. Compare `bash/.bashrc` against the current Omarchy Bash defaults, in the reference clone under `omarchy/default/` and installed under `/usr/share/omarchy/default/`:
   - the upstream preamble (everything above `# Personal overrides`) against `default/bashrc`, the seed Omarchy installs as `/etc/skel/.bashrc`; it is kept verbatim except for the seed's closing placeholder comment, which the `# Personal overrides` section replaces, so adopt upstream changes to the rest
   - AI launch aliases (`c`, `cx`, `cy`, `ic`, `ix`, and `icx`) stay sourced unchanged from `default/bash/aliases`, with no `unalias` override. Stock `cx` and `hdw cc` both start Claude Code; both launch paths use normal client configuration, with stock flags only on stock launches
   - `y()` is additive (Yazi is not in Omarchy)
   - the sourced `hdw` twin against `default/bash/fns/herdr`: it stays additive, creating/focusing a new workspace in the current physical directory inside an already-running Herdr, with AI-left/Neovim-top-right/shell-bottom-right geometry and AI focus. Valid populated/inactive callers, repeated new workspaces and generated-shell chaining are supported; preserve existing names/layouts apart from global focus. Naming stays native, without `--label`; `cc`/`oc` are arguments sending full commands, with optional `-c`, not a separate client profile. No reuse/registry/startup/attach behavior. A change lands in EyrWSL in the same session (`make twins`)
...
5. Compare `system/` against what Omarchy installs on this host: search `/usr/share/omarchy` for `sysrq`, `dkms` and the NVIDIA driver packages it selects. A new Omarchy SysRq setting, or a move away from `nvidia-open-dkms`, needs review against the Host section of `DEVIATIONS.md`; `make verify` checks the installed copies and the loaded patch
6. App parity sweep: diff `pacman -Qe` against the installed default manifest (`/usr/share/omarchy/install/omarchy-base.packages` plus hardware conditionals) and the optional installers (`omarchy-install-*`); classify each extra as personal, optional-installed, or retired survivor, and account for provider resolution (`extra/neovim` satisfies the `nvim` entry)
7. Tool-path integrity: compare Omarchy-owned launchers in `~/.local/bin` with the installer definitions that wrote them, then check `mise ls --current` and actual command resolution. Review any replacement or native-store removal against its actual owner and running executable; directory location alone is not disposal authority
8. Webapp entries: compare the webapp launchers in `~/.local/share/applications` against the current Omarchy default set and remove stale ones with `omarchy-webapp-remove`; personal bindings launch by URL and do not depend on desktop entries
9. Cross-repo coordination: read the other host repository's ledger for items assigned to this repo and for stale entries describing this host's environment
10. For each difference, classify it:
