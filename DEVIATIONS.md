...
- The `t`/`h` prefix follows Omarchy's multiplexer lettering. These helpers are additive alongside Omarchy's `tdl`/`tds` and `hdl`/`hds`; `tdw`, `hdw`, and their declared tests/fixtures are byte-identical twins with EyrWSL.
