...
- `.bashrc` opens with the upstream 4.0.2 preamble (sources `default/bash/env-bootstrap` when readable, so `OMARCHY_PATH` and the mise and `~/.local/bin` `PATH` entries reach non-interactive shells too, returns for non-interactive shells, then sources `$OMARCHY_PATH/default/bash/rc`), then adds personal overrides below. The preamble is upstream-authored and kept verbatim; Omarchy writes to `.bashrc` reach the repo file through the stow symlink.
...
- Claude Code launches use the supported maximum effort, `--effort max`. The `claude` alias applies it to every interactive launch, including Omarchy's `cx` alias (`claude --permission-mode auto` since 4.0.1; the effort flag composes with it through alias expansion) and `tdl`-launched AIs; scripts, hooks, and the pinned `spar-claude` bridge stay plain because they do not expand aliases. This intentionally overrides the `CLAUDE_CODE_EFFORT_LEVEL=xhigh` pin that EyrAgents tracks in its Claude settings `env` for interactive Omarchy sessions.
...
- `monitors.lua` is tracked: the GU605C panel at `2560x1600@240`, scale 1.6, and `GDK_SCALE=2`. The NVIDIA client env comes from the shipped `default/hypr/nvidia.lua` detection. `hl.env` values reach the compositor and its direct spawns on reload; uwsm-launched clients pick them up at session start.
